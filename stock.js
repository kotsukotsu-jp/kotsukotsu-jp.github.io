...
(function () {
  ...
  fetch("../data/stocks/" + code + ".json", { cache: "no-store" })
    ...
    .then(function (d) {
      var years = d.years || [];
      var lbl = function (r) { return ((r && r.fiscal_year) || "").replace("-", "/"); };
      var esc = window.Chart.esc;
      var pts = function (f) {
      ...
      /* 1株あたりの金額は、株式分割・併合をまたぐと比べられない。
         切れ目の判定は chart.js（Chart.perShareCut）に置いてある。
         **検査（tools_audit_data.py）が同じ関数を呼ぶため**で、
         ここに書くと「利用者に見えている系列」を機械で確かめられなくなる。 */
      var perShareBlock = function (title, f, opts) {
        var c = window.Chart.perShareCut(years, f);
        var o = Object.assign({}, opts);
        if (c > 0) {
          o.note = esc(lbl(years[c])) + "期に株式分割または併合があり、" +
            "1株あたりの基準が変わりました。前後は比べられないため、" +
            "<b>それ以降だけ</b>を出しています（それ以前は有価証券報告書でご確認ください）。";
        }
        return window.Chart.block(title, pts(f).slice(c), o);
      };
      ...
      out += '<h3>業績</h3><div class="grid2">' +
        ...
        perShareBlock("1株配当（円）", "dps", { zeroBase: true, digits: 1 }) +
        window.Chart.block("自己資本比率（%）", pts("equity_ratio"), { type: "line", digits: 1 }) +
        perShareBlock("EPS（円）", "eps", { type: "line", digits: 1 }) +
        perShareBlock("BPS（円）", "bps", { type: "line", digits: 1 }) +
