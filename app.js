...
(function () {
  ...
  function renderList() {
    ...
    if (state.tab === "candidate" && state.page === 1 && !state.q) {
      ...
      if (top3.length >= 2) {
        /* クラス spa は「SPAが描いたほう」の目印。狭い画面では
           下のカード一覧が同じ3社を同じ見た目で繰り返すため、これを隠す。
           静的HTML側（JSが動かない環境・クローラー）の .ccards は
           表が4件目以降しか持たないので隠してはならない。 */
        topCards = '<div class="ccards spa">' + top3.map(function (j) {
