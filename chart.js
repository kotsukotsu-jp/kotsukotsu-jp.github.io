...
(function (global) {
  ...
  function block(title, points, opts) {
    ...
    return '<div class="chartbox">' +
      ...
      /* グラフ固有のただし書き。「なぜこの期間だけなのか」はグラフの
         すぐ横に無いと読まれない（離れた場所に書くと結び付かない） */
      (opts.note ? '<p class="count">' + opts.note + "</p>" : "") +
      "</div>";
  }

  /* --- 1株あたりの金額は、株式分割・併合をまたぐと比べられない -------------
     有価証券報告書は、分割前の期の1株配当・EPS・BPSを**当時の実額のまま**
     載せることがある。6196 は BPS が 131,570円 → 45円（約3千分の1）に落ちる。
     そのまま1本のグラフに並べると軸が3千倍に引き伸ばされ、**直近10年ぶんが
     底に貼り付いて0に見える**（利用者からの指摘。実測で1,232銘柄）。

     株数の比で分割後の金額に直すのは、実データで58%しか裏づけが取れなかった
     ため行わない。**推測で数字を作らず、切れ目より前を出さない。**

     切れ目とみなすのは
       (a) BPSが5倍以上動いた、または発行済株式数が1.8倍以上動いた期で、
       (b) **その系列自身も5倍以上動いている**とき
     の両方がそろった場合だけ。赤字からの回復で利益が大きく振れただけの期
     （昭文社の EPS -395 → 30 など）を切り落とさないため、(a) を必ず求める。

     **ここに置くのは検査から呼ぶため。** 描画する側にだけ書くと、
     「利用者に見えている系列」を機械で確かめる手段が無くなる。 */
  var REBASE_BPS = 5, REBASE_SHARES = 1.8, REBASE_VALUE = 5;

  function jumped(a, b, lim) {
    if (!a || !b) return false;
    var r = b / a;
    return r < 0 || r >= lim || r <= 1 / lim;
  }

  /* 期 i の手前に「1株あたりの基準が変わった」裏づけがあるか */
  function rebasedAt(years, i) {
    if (i < 1) return false;
    var p = years[i - 1], c = years[i];
    return jumped(p.bps, c.bps, REBASE_BPS) ||
           jumped(p.shares_outstanding, c.shares_outstanding, REBASE_SHARES);
  }

  /* field の系列を、どの位置から出せばよいか（0 なら全期出してよい） */
  function perShareCut(years, field) {
    years = years || [];
    var c = 0;
    for (var i = 1; i < years.length; i++) {
      if (rebasedAt(years, i) && jumped(years[i - 1][field], years[i][field], REBASE_VALUE)) {
        c = i;
      }
    }
    return c;
  ...
  global.Chart = { svg: svg, table: table, block: block, bindToggles: bindToggles,
                   fmt: fmt, esc: esc, perShareCut: perShareCut,
                   rebasedAt: rebasedAt, jumped: jumped };
})(window);
